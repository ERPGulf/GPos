...
def get_pos_pdf_mapping():
    file_doc = frappe.get_all(
        "File",
        filters={"attached_to_doctype": "Company", "attached_to_name": "HIKIT TRADING"},
        fields=["file_url"]
    )

    if not file_doc:
        frappe.msgprint("No PDF Mapping JSON found in Company attachments for HIKIT TRADING.")
        return {}
    ...
    try:
        ...
        frappe.msgprint(" Loaded JSON Template Successfully from Company Attachment:", json_file_url)
        return pdf_mapping
    except Exception as e:
        frappe.msgprint(" Error Loading JSON Template:", e)
        return {}
...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        if extracted_text.strip():  
            ...
            frappe.msgprint(extracted_text)
            return extracted_text
    except Exception as e:
        frappe.msgprint(" Error using pdfplumber, switching to OCR:", e)
...
def extract_address_details(text, start_keyword, end_keywords):
    ...
    if match:
        ...
        city = [full_address[-2]] if len(full_address) >= 2 else []
        country = [full_address[-1]] if len(full_address) >= 1 else []
...
def extract_invoice_details_from_text(extracted_text, pdf_mapping):
    ...
    frappe.msgprint("\n--- Extracting Fields from PDF Text ---")
    for key, pattern in pdf_mapping.items():
        frappe.msgprint(f"Extracting {key} using regex: {pattern}")
        ...
        if isinstance(pattern, dict):  
            ...
            if key == "supplier":
                supplier_address, supplier_city, supplier_country = extract_address_details(
                    extracted_text, "t/a CHANGA SAFARI CAMP", ["HIKIT TRADING", "TIN NO"]
                )
    ...
    frappe.msgprint("\n Extracted Invoice Details:", invoice_details)
...
def extract_line_items(extracted_text, pdf_mapping):
    ...
    if not line_pattern:
        frappe.msgprint("⚠️ No line items pattern found in the template.")
        return []

    frappe.msgprint("\n Extracting Line Items using pattern:", line_pattern)
    ...
    for match in matches:
        ...
        for i, field in enumerate(fields):
            if i < len(match):  # Ensure index exists
                item[field] = safe_float(match[i]) if "Price" in field or "Quantity" in field else match[i].strip()
        line_items.append(item)

    if not line_items:
        frappe.msgprint("⚠️ No line items extracted. Please check the regex pattern and extracted text.")
...
def save_invoice_data(pdf_path, output_json_path):
    pdf_mapping = get_pos_pdf_mapping()

    if not pdf_mapping:
        frappe.msgprint(" No valid PDF mapping found. Exiting...")
        return
